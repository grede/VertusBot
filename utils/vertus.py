...
class Vertus:
    ...
    async def stats(self):
        ...
        referrals = (await (await self.session.post('https://api3.thevertus.app/users/get-referrals/1', json={})).json()).get('total')
    ...
    async def claim_daily_reward(self):
        resp = await self.session.post("https://api3.thevertus.app/users/claim-daily", json={})
    ...
    async def upgrade(self, upgrade):
        ...
        resp = await self.session.post('https://api3.thevertus.app/users/upgrade', json=json_data)
        resp_json = await resp.json()

        return resp.status, self.from_nano(resp_json.get('newBalance')) if resp_json.get("success") else await resp.text()

    async def collect(self):
        resp = await self.session.post('https://api3.thevertus.app/game-service/collect', json={})
    ...
    async def first_collect(self):
        resp = await self.session.post('https://api3.thevertus.app/game-service/collect-first', json={})
        return self.from_nano((await resp.json()).get('newBalance'))

    async def get_data(self):
        resp = await self.session.post('https://api3.thevertus.app/users/get-data', json={})
        return (await resp.json()).get('user')

    async def create_wallet(self):
        resp = await self.session.post('https://api3.thevertus.app/users/create-wallet', json={})
        return (await resp.json()).get('walletAddress')

    @staticmethod
    def get_upgrades(data):
        return data.get('abilities').get('farm'), data.get('abilities').get('population')
